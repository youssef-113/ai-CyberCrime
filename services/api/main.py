...
from .middleware import get_current_user, get_current_user_id, get_session_tenant, require_session, get_optional_user
...
async def verify_evidence(
    ...
    sanitized_evidence_text = sanitize_string(request.evidence_text, max_length=16000)
    sanitized_request = {
        "extracted_entities": sanitize_payload(request.extracted_entities),
        "classification": sanitize_payload(request.classification),
        "retrieved_articles": sanitize_payload(request.retrieved_articles),
        "evidence_blocks": sanitize_payload(request.evidence_blocks or []),
    }
...
async def chat(
    ...
    session_id = request.session_id
    user_message = sanitize_string(request.user_message, max_length=2000)
    case_context = sanitize_payload(request.case_context or {})
...
async def chat_pdf_trigger(
    ...
    try:
        resp = await call_microservice(
            "chatbot",
            "POST",
            f"{SERVICE_URLS.get('chatbot', 'http://chatbot:8006')}/chat/pdf_trigger",
...
async def retrieve_articles(
    ...
    safe_query = sanitize_string(request.query, max_length=2000)
    safe_crime_type = sanitize_string(request.crime_type, max_length=100)
    safe_transform_strategy = sanitize_string(request.transform_strategy, max_length=50)
    safe_tenant_id = sanitize_string(request.tenant_id, max_length=100)
...
async def classify_text(
    ...
    safe_text = sanitize_string(request.text, max_length=20000)
    safe_entities = sanitize_payload(request.entities)
...
async def check_faithfulness(
    ...
    safe_query = sanitize_string(request.query, max_length=2000)
    safe_answer = sanitize_string(request.answer, max_length=2000)
    safe_citations = sanitize_payload(request.citations)
...
async def index_articles(
    ...
    safe_tenant_id = sanitize_string(request.tenant_id, max_length=100)
    safe_articles = sanitize_payload(request.articles)
